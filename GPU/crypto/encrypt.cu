...
__global__ void encryptKernel(char* deviceDataIn, char* deviceDataOut) {
    int KEY[5] = {0,1,2,3,4};
    int LEN_KEY = sizeof(KEY)/sizeof(KEY[0]); 
    unsigned index = blockIdx.x * blockDim.x + threadIdx.x;
    deviceDataOut[index] = deviceDataIn[index] + KEY[index % LEN_KEY];
    if (deviceDataOut[index] > 127)
        {
	deviceDataOut[index] -= 128;
        }	
...
__global__ void decryptKernel(char* deviceDataIn, char* deviceDataOut) {
    int KEY[5] = {0,1,2,3,4};
    int LEN_KEY = sizeof(KEY)/sizeof(KEY[0]);
    int renormalized_data_in = 0; 
    unsigned index = blockIdx.x * blockDim.x + threadIdx.x;
    deviceDataOut[index] = deviceDataIn[index] - KEY[index % LEN_KEY];

    if (deviceDataIn[index] - KEY[index % LEN_KEY] >= 0)
	{
	//cout << "Case 1" << endl;
	deviceDataOut[index]=deviceDataIn[index] - KEY[index % LEN_KEY];
	
	//cout << int(data_in[i])<< endl;
	//cout << "-" <<endl;
	//cout <<int( data_out[i]) << endl;
	//cout <<"---------"<<endl;
	}
	...
	{
	//cout << "Case 2" <<endl;
	renormalized_data_in = deviceDataIn[index] + 128;
        deviceDataOut[index] = renormalized_data_in - KEY[index % LEN_KEY];  
	}
}
...
int fileSize() {
  ...
  ifstream file ("file.data", ios::in|ios::binary|ios::ate);
  if (file.is_open())
  {
    size = file.tellg();
    file.close();
...
{  
  ...
  for (i=0; i<n; i++) { data_out[i]=data_in[i] + KEY[i % LEN_KEY];
	if (data_out[i] > 127){
	data_out[i] -= 128;
}	
	//cout << int(data_in[i])<< endl;
...
if (data_in[i] - KEY[i % LEN_KEY] >= 0)
	{
	//cout << "Case 1" << endl;
	data_out[i]=data_in[i] - KEY[i % LEN_KEY];
	...
	{
	//cout << "Case 2" <<endl;
	renormalized_data_in = data_in[i] + 128;
        data_out[i] = renormalized_data_in - KEY[i % LEN_KEY];  
	}
}
  sequentialTime.stop();
...
int main(int argc, char* argv[]) {
    ...
    cout << "Decrypting a file of " << n << "characters using Sequential Programming" << endl;
    DecryptSeq(n, data_in, data_out);
    writeData(n, "sequential_decrypted.data", data_out);

    // CUDA block decryption
    readData("cuda.data", data_in);
    cout << "Decrypting a file of " << n << "characters using CUDA Programming" << endl;
